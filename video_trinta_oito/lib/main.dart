...
class MyAppWidgets extends StatefulWidget {
  const MyAppWidgets({Key? key}) : super(key: key);

  @override
  State<MyAppWidgets> createState() => _MyAppWidgetsState();
}

class _MyAppWidgetsState extends State<MyAppWidgets> {
  @override
  Widget build(BuildContext context) {
    Widget showText() => const Text('Imagem interna:');

    Widget showInternalImage() => Image.asset(_InternalImage);

    Widget showText1() => const Text('Imagem da net');

    Widget showExternalImage() => Image.network(
        'https://v5j9q4b5.rocketcdn.me/wp-content/uploads/2020/08/meme-pra-que-quem-e-o-garoto-das-fotos-e-origem-da-brincadeira-1024x655.png.webp');

    Widget showTextByteImage() => const Text('Imagem em Byte64');

    Widget showExternalByteImage() => Image.memory(_ByteImage);
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          showExternalImage(),
          showTextByteImage(),
          showExternalByteImage(),
        ],
      ),
    );
  }

  final _ByteImage = const Base64Decoder().convert(
  ...
  String _InternalImage = "images/android_ios.png";
}
